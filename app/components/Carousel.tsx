...
const Carousel = () => {
  let imagesArray = images.map((image, index) => ({ ...image, id: index }));

  const [currentSlide, setCurrentSlide] = useState<any>(imagesArray[0]);
  const slideRef = useRef<number>(0);

  const showNextSlide = () => {
    slideRef.current = (slideRef.current % imagesArray.length) + 1;
    if (slideRef.current == imagesArray.length) {
      slideRef.current = 0;
    }
    console.log(slideRef.current);
    let img = imagesArray.filter((image) => image.id == slideRef.current);
  ...
  const showPrevSide = () => {
    ...
    if (slideRef.current == -1) {
      slideRef.current = imagesArray.length - 1;
    }
    let img = imagesArray.filter((image) => image.id == slideRef.current);
